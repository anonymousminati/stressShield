...
class NewExercise3 extends StatefulWidget {
  const NewExercise3({super.key, required  this.selectedGoal, required  this.selectedDuration});
...
class _NewExercise3State extends State<NewExercise3> {
  ...
  Future<void> fetchMusics() async {
    try {
      final querySnapshot = await FirebaseFirestore.instance.collection('exerciseaudio').doc(widget.selectedGoal).get();
      ...
      if (musicsData != null) {
        ...
        setState(() {
          musics = []; // Initialize musics as an empty list if musicsData is null
        });
      }
    } catch (e) {
      setState(() {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error fetching musics. Please check your internet connection and try again.'),
          action: SnackBarAction(
            label: 'Retry',
            onPressed: fetchMusics, // Retry fetching musics when the user taps Retry
          ),
        ),
      );
    }
  ...
  Widget build(BuildContext context) {
    return musics == null ? CircularProgressIndicator(): SafeArea(
        ...
        child: Scaffold(
          ...
          floatingActionButton: FloatingActionButton.extended(
            ...
            onPressed: () {
              Navigator.push(
                context,
                MaterialPageRoute(builder: (context) => BreathExercise(  selectedGoal: widget.selectedGoal, selectedDuration: widget.selectedDuration,
                  audioUrl:audio_url,)),
              );
            },
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              Container(
                                ...
                                child: Lottie.asset('assets/lottie/sound_wave.json', fit: BoxFit.fill,width: MediaQuery.of(context).size.width, height: 300,),
                              ),
                                  ...
                                  itemBuilder: (context, index) {
                                    ...
                                    return GestureDetector(
                                      ...
                                      child: Container(
                                        ...
                                        decoration: BoxDecoration(
                                          color: _selectedIndex == index ? Color(0xFFED7E1C) : Colors.transparent,
                                          borderRadius: BorderRadius.circular(25),
                                          border: Border.all(
                                            color: _selectedIndex == index ? Color(0xFFF5A259).withOpacity(0.8) : Color(0xFF4F3422),
                                        ...
                                        child: Text(
                                          music['title'],
                                          style: TextStyle(
                                            color: _selectedIndex == index ? Colors.white : Color(0xFF4F3422),
                                            fontSize: 16,
                                            fontWeight: FontWeight.bold,
                                          ),
                                        ),
                                    ...
                                    : Center(child: CircularProgressIndicator()), // Show loading indicator while fetching musics
                              ),
